...
class CommonViewsTestCase(PermaTestCase):
    ...
    def test_screenshot_only_archive_not_screenshot_view(self):
    ...
    def test_capture_only_archive_default_screenshot_view(self):
    ...
    def test_screenshot_only_archive_default_screenshot_view(self):
    ...
    # This tests that where there is ONLY a primary and no default to screenshot, there's no redirect to add the "type=standard" query
    def test_screenshot_only_archive_default_standard_view(self):
    ...
    def test_full_archive_default_screenshot_view(self):
    ...
    def test_capture_only_default_screenshot_view(self):
        link = Link.objects.get(guid='N1N0-33DB')
        self.assertTrue(link.screenshot_view)
        with patch('perma.models.default_storage.open', lambda path, mode: open(os.path.join(settings.PROJECT_ROOT, 'perma/tests/assets/new_style_archive/archive.warc.gz'), 'rb')):
            response = self.get('single_permalink', reverse_kwargs={'kwargs':{'guid': 'N1N0-33DB'}}, request_kwargs={'follow': True})
