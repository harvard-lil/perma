...
def single_permalink(request, guid):
    ...
    if context['can_view'] and link.can_play_back():
        ...
        context['view_mode'] = request.GET.get('view-mode', 'client-side')
        context['view_action'] = request.GET.get('view-action', 'replay')

        if context['view_mode'] == 'server-side':
            # 'Legacy' play back using Webrecorder
            try:
                logger.info(f"WR playback: {link.guid}")
            ...
            logger.debug(f'Client-Side playback prep: {link.guid}')
            if context['view_mode'] == 'compare':
                if request.user.is_anonymous or not request.user.offer_compare_playback:
                    context['view_mode'] = 'client-side'
                    logger.info(f"{'UID: ' + str(request.user.pk) if not request.user.is_anonymous else 'Anon'} denied "
                                f"compare on link {link.guid}.")
                else:
                    logger.info(f'Playback Comparison: {link.guid}')
                    context['view_action'] = 'replay'
                    context.update({
                        'wr_host': settings.PLAYBACK_HOST,
            ...
            context.update({
                ...
                'view_action': context['view_action'],
            })
...
# sspbdefault remove when when client_side_playback is the default
@if_anonymous(cache_control(max_age=settings.CACHE_MAX_AGES['single_permalink']))
@ratelimit(rate=settings.MINUTE_LIMIT, block=True, key=ratelimit_ip_key)
@ratelimit(rate=settings.HOUR_LIMIT, block=True, key=ratelimit_ip_key)
@ratelimit(rate=settings.DAY_LIMIT, block=True, key=ratelimit_ip_key)
def single_permalink_sspbdefault(request, guid):
    """
    Given a Perma ID, serve it up.
    """
    raw_user_agent = request.META.get('HTTP_USER_AGENT', '')
    ...
    logger.debug(f"Rendering template for {link.guid} because we warc hard for the money")
    response = render(request, 'archive/single-link_sspbdefault.html', context)
