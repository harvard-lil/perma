...
from perma.models import Registrar, Organization
...
class FederalCourtOrganizationManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            registrar__in=FederalCourt.objects.all()
        )


class FederalCourtOrganization(Organization):

    class Meta:
        proxy = True
        verbose_name = "Federal Court Organization"

    objects = FederalCourtOrganizationManager()
