...
mkcert -key-file services/docker/s3/ssl/private.key -cert-file services/docker/s3/ssl/public.crt perma.minio.test
...
# For dev convenience: restart s3 and wacz-exhibitor (which will have crashed immediately) now that the required cert and key files are available
